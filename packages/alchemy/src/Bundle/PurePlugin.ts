...
export interface PurePluginOptions {
   ...
   * configuration.
   *
   * Skipped when the owning `package.json` declares `sideEffects` other
   * than `false` / `[]`, since adding pure annotations to a package the
   * author has marked as having init-time side effects could change
   * runtime behavior.
   *
   * @default true
   */
...
export const purePlugin = (
  ...
  // Per-bundle cache of directory -> owning package name (or null).
  ...
  return {
    ...
    transform: {
      ...
      async handler(code, id) {
        const pkg = await resolvePackageName(id, pkgNameCache);
        if (pkg === null || !isMatch(pkg)) return null;
        // Never mark entries as side-effect free; see `entryPaths` note.
        const cleanId = id.replace(/[?#].*$/, "");
        const isEntry = entryPaths.has(cleanId);
        const markSideEffectFree = markSideEffectFreeOpt && !isEntry;
...
async function detectEntryPackage(
  ...
  for (const candidate of inputs) {
    ...
    if (!isSideEffectFree(meta.sideEffects)) continue;
    return meta.name;
  }
  return null;
}
...
/**
 * Resolves the owning npm package name of a module id, using the path-based
 * `packageNameFromId` first and then falling back to an asynchronous walk
 * up the directory tree until a `package.json` is found. This second path
 * is what makes the plugin work for workspace-linked sources (e.g. our own
...
export async function resolvePackageName(
  ...
  // cache entries with the resolved name once we find one.
  ...
  for (let i = 0; i < 64; i++) {
    ...
    if (name !== null) {
      for (const v of visited) cache.set(v, name);
      return name;
    }
    const parent = path.dirname(dir);
    if (parent === dir) break;
